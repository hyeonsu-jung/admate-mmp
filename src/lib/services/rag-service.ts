...
[출력 포맷 특별 지침]
- **기술 파라미터(Parameter) 목록**을 설명할 때는 반드시 아래의 JSON 구조를 마크다운 코드블록(\`\`\`json)으로 포함하세요.
- JSON 구조 예시:
\`\`\`json
{
  "type": "parameter_list",
...
\`\`\`
- 일반 텍스트 설명 중에 파라미터가 언급될 때는 \`key\` 형식을 사용하세요.
- 마크다운 리스트 작성 시 \`항목명 (key)\` 형태를 한 줄에 작성하고, 괄호가 다음 줄로 넘어가지 않도록 주의하세요.
`;
    }
}
